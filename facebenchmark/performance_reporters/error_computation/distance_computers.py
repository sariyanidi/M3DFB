...
class DenseP2PDistance(BaseDistanceComputer):
    ...
    def compute(self, X, Y, pidx, Xlmks=None, Ylmks=None):
        ...
        -------
        ndarray
            1D array containing errors per vertex.

        """
        return np.sqrt((((X-Y[pidx])**2).sum(axis=1)))
...
class DenseP2TriDistance(BaseDistanceComputer):
    ...
    def compute(self, X, Y, pidx, Xlmks=None, Ylmks=None):
        ...
        err = np.zeros(N)
        for i in range(N):
            tidx = np.argsort(np.sqrt((((Yy[i,:]-Yy)**2).sum(axis=1))))[0:3] # select 3-NN for each point in G. The 0-th is the point itself
            err[i], _ = ptd.pointTriangleDistance(Yy[tidx,:],X[i,:])
        return err
...
class LandmarksDistance(BaseDistanceComputer):
    ...
    def compute(self, X, Y, pidx, Xlmks, Ylmks):
        ...
        """
        return np.sqrt((((Xlmks-Ylmks)**2).sum(axis=1)))
